...
function downloadTemplate() {
  ...
  a.href = url;
  a.download = "balance_template.csv";
  a.click();
  URL.revokeObjectURL(url);
}

export function BulkUploadBalancePage() {
  const { user } = useAuth();
  ...
  async function handleUpload() {
    ...
    setUploadError(null);
    try {
      const data = await bulkUploadBalance(selectedFile);
      setResult(data);
    } catch (err) {
      ...
      <div className="max-w-3xl mx-auto py-6 sm:px-6 lg:px-8 space-y-6">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-600">
            Upload a CSV file to set stock balances in bulk. Creates
            COUNT_ADJUSTMENT movements for rows where the current balance
            differs from the target quantity.
          </p>
        </div>
          ...
          <div className="bg-white shadow rounded-lg p-6 space-y-4">
            ...
            <div className="flex gap-6 text-sm">
              <div className="flex flex-col items-center bg-green-50 rounded-lg px-6 py-3">
                ...
                <span className="text-green-800">Processed</span>
              </div>
              <div className="flex flex-col items-center bg-amber-50 rounded-lg px-6 py-3">
                <span className="text-2xl font-bold text-amber-700">
                  {result.skipped}
                </span>
                <span className="text-amber-800">Skipped (no change)</span>
              </div>
